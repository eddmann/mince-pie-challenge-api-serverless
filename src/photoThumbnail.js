...
const resizeAndAssign = record => {
  ...
  const resize = s3.getObject({ Bucket: bucket, Key: uploadKey }).promise()
...
export const photoThumbnail = event =>
  Promise.all(event.Records.map(resizeAndAssign)).then(console.log);
